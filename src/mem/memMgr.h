...
class MemRecycleList
{
   ...
   T* popFront() {
	  ...
	  T* pop_Ele = _first;
      void* temp = pop_Ele;
	  size_t* front_8_Byte = (size_t*) temp;
	  _first = (T*)*front_8_Byte;
	  *front_8_Byte = 0;
      return pop_Ele;
   }
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      // TODO
      for (int i = 0; i < R_SIZE; ++i){
		 MemRecycleList<T>* temp = _recycleList[i]._nextList;
		 ...
		 _recycleList[i]._nextList = NULL;
	  }
	  MemBlock<T>* temp_memblock = _activeBlock;
	  while(temp_memblock -> _nextBlock != NULL){
	     _activeBlock = temp_memblock -> _nextBlock;
		 delete temp_memblock;
		 temp_memblock = _activeBlock;
	  }
	  if(b == 0)
        _activeBlock -> reset();
	  else{
      ...
      if(ret == NULL){
	     if(!(_activeBlock -> getMem(t,ret))){
			if(_activeBlock -> getRemainSize() >= toSizeT(S)){ 
		       size_t i = getArraySize(_activeBlock -> getRemainSize());
			   void* temp = _activeBlock -> _ptr;
			   T* remain_memory = (T*)temp;
               getMemRecycleList(i) -> pushFront(remain_memory); 
               #ifdef MEM_DEBUG
               cout << "Recycling " << ret << " to _recycleList[" << n << "]\n";
               #endif // MEM_DEBUG
			}
            MemBlock<T>* next = new MemBlock<T>(_activeBlock,_blockSize);
			_activeBlock = next;
            #ifdef MEM_DEBUG
            cout << "New MemBlock... " << _activeBlock << endl;
            #endif // MEM_DEBUG
